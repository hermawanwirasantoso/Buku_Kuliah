package com.example.bukukuliah;

public class FirebaseHelper {

    public static final String COLLECTION_BUKU = "buku";
    public static final String JUDUL_BUKU = "judul_buku";
    public static final String DESKRIPSI_BUKU = "deskripsi_buku";

    public static final String COLLECTION_CATATAN = "catatan";
    public static final String CONTENT_CATATAN = "content";
    public static final String PAGE_CATATAN = "page";
    public static final String TANGGAL_CATATAN = "tanggal";
    public static final String HTML_CATATAN = "html";

    public static final String COLLECTION_JADWAL = "jadwal";
    public static final String DOCUMENT_JADWAL = "user_jadwal";

    public static final String JUDUL_KEGIATAN = "judul_kegiatan";
    public static final String DESKRIPSI_KEGIATAN = "deskripsi_kegiatan";
    public static final String LOKASI_KEGIATAN = "lokasi_kegiatan";
    public static final String TANGGAL_KEGIATAN = "Tanggal_kegiatan";
    public static final String WAKTU_KEGIATAN_START = "Waktu_kegiatan_START";
    public static final String WAKTU_KEGIATAN_END = "Waktu_kegiatan_END";
    public static final String JADWAL_ID = "jadwal_id";
    public static final String EVENT_ID="event_id";
    public static final String DAY_OF_WEEK="day_of_week";

    public static final String COLLECTION_USERS = "users";
    public static final String USERS_TIMESTAMP = "user_last_login";

    public static final String COLLECTION_GAMBAR = "gambar";
    public static final String URL_GAMBAR = "url_gambar";
    public static final String DESC_GAMBAR = "desc_gambar";
    public static final String TIMESTAMP_GAMBAR = "timestamp_gambar";
    public static final String PAGE_GAMBAR = "page_gambar";
    public static final String FILENAME_GAMBAR = "filename_gambar";

    public static final String COLLECTION_AUDIO = "audio";
    public static final String URL_AUDIO = "url_audio";
    public static final String DESC_AUDIO = "desc_audio";
    public static final String TIMESTAMP_AUDIO = "timestamp_audio";
    public static final String PAGE_AUDIO = "page_audio";
    public static final String FILENAME_AUDIO = "filename_audio";


    public static final String STORAGE_SAVED_USERS = "users/";
}
